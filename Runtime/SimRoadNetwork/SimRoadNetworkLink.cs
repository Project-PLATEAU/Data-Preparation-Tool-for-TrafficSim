...
{
    /// <summary>
    /// シミュレーション用道路ネットワークのリンクを表すクラス
    /// </summary>
    [System.Serializable]
    public class SimRoadNetworkLink
    {
        ...
        public SimRoadNetworkNode UpNode { get; set; }

        [field: SerializeField, SerializeReference]
        public SimRoadNetworkNode DownNode { get; set; }

        [field: SerializeField, SerializeReference]
        public SimRoadNetworkLink Pair { get; set; }
        ...
        public SimRoadNetworkLink(string id, int index)
        {
            ID = IDPrefix + id;

            RoadNetworkIndex = index;

            //　Reverse判定がまだ行われていないため個々では行わない
        }
        ...
        public RnDataRoad OriginLink
        {
            get
            {
                ...
                return SimRoadNetworkManager.RoadNetworkDataGetter.GetRoadBases()[RoadNetworkIndex] as RnDataRoad;
            }
        }
                ...
                if (runtimeTrans == null)
                {
                    ...
                    else
                    {
                        ...
                        foreach (var lane in Lanes)
                        {
                            ...
                            var points = geo.Select(p => SimRoadNetworkManager.GeoReference.Project(new GeoCoordinate(p.Latitude, p.Longitude, 0))).ToList();
        ...
        [field: SerializeField]
        public List<SimRoadNetworkLane> Lanes = new List<SimRoadNetworkLane>();

        public void GenerateLane()
        {
            Lanes = new List<SimRoadNetworkLane>();

            if (OriginLink != null)
            {
                var lanesall = SimRoadNetworkManager.RoadNetworkDataGetter.GetLanes();
                ...
                for (int i = 0; i < lanes.Count; i++)
                {
                    ...
                    Lanes.Add(new SimRoadNetworkLane(ID.Replace(SimRoadNetworkLink.IDPrefix, ""), lane, i));
                }
            }
        }
        ...
        public List<GeoJSON.Net.Geometry.Position> GetGeometory()
        {
            ...
            if (Lanes.Count == 0)
            {
                ...
                var upGeoCood = SimRoadNetworkManager.GeoReference.Unproject(new PlateauVector3d(upCoord.x, upCoord.y, upCoord.z));
                var downGeoCood = SimRoadNetworkManager.GeoReference.Unproject(new PlateauVector3d(downCoord.x, downCoord.y, downCoord.z));
        ...
        public List<RnID<RnDataLane>> GetOriginLanes()
        {
            ...
            var laneAll = SimRoadNetworkManager.RoadNetworkDataGetter.GetLanes();
